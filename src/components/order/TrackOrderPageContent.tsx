...
function TrackOrderInner() {
  ...
  return (
    <div className="max-w-250 mx-auto px-4 md:px-6 lg:px-8 py-10">
      {/* Breadcrumb */}
      ...
      <div className="bg-white rounded-3xl p-6 md:p-8 border border-zinc-100 shadow-sm max-w-xl mx-auto mb-10">
        <form onSubmit={handleTrack} className="flex flex-col sm:flex-row gap-3">
          <input
            ...
            className="flex-1 px-4 py-3.5 border border-zinc-200 rounded-xl text-sm uppercase outline-none focus:border-crimson"
            required
          />
          <button
            type="submit"
            className="px-6 py-3.5 bg-crimson text-white font-bold rounded-xl shadow-md hover:bg-[#991B1B] transition-colors text-sm cursor-pointer"
          >
            Track Status
          </button>
        </form>
        ...
        <div className="flex items-center justify-between mt-3 text-xs text-zinc-400">
          <span>Need sample ID? Try: <button type="button" onClick={() => setOrderInput("SGM-739102")} className="text-crimson underline font-medium">SGM-739102</button></span>
        </div>
      </div>
          ...
          <div className="space-y-8 relative before:absolute before:inset-0 before:left-4 before:h-full before:w-0.5 before:bg-zinc-200">
            {TIMELINE_STEPS.map((step, idx) => (
              <div key={idx} className="relative flex items-start gap-5 group">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 z-10 ${step.done
                    ? step.current
                      ? "bg-crimson text-white ring-4 ring-red-100 animate-bounce"
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <h3
                      className={`text-sm font-bold ${step.done ? "text-zinc-900" : "text-zinc-400"
                        }`}
                    >
                      {step.title}
                    </h3>
