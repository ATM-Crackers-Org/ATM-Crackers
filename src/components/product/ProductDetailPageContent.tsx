  ...
  return (
    <>
      <main className="bg-warm-white min-h-screen">
        <div className="max-w-360 mx-auto px-4 md:px-6 lg:px-8 py-8">
          ...
          <nav className="flex items-center gap-2 text-xs text-zinc-400 mb-6">
            ...
            <span>›</span>
            <Link href="/shop" className="hover:text-crimson">Shop</Link>
            <span>›</span>
            <Link href={`/categories/${product.category_slug}`} className="hover:text-crimson">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-4 bg-crimson text-white font-bold rounded-2xl hover:bg-[#991B1B] shadow-[0_4px_20px_rgba(185,28,28,0.35)] transition-all text-sm cursor-pointer"
                >
                  🛒 ADD TO CART
                </button>
                <Link
                ...
                <button
                  ...
                  className={`w-14 rounded-2xl flex items-center justify-center text-2xl border-2 transition-all cursor-pointer ${wishlisted ? "bg-pink-50 border-pink-300 text-pink-500" : "border-zinc-200 text-zinc-400"
                    }`}
                >
                  {wishlisted ? "♥" : "♡"}
                </button>
      ...
      <div className="fixed bottom-16 left-0 right-0 z-40 md:hidden bg-white border-t border-zinc-200 px-4 py-3 flex gap-3">
        <button
          onClick={handleAddToCart}
          className="flex-1 py-3 bg-crimson text-white text-sm font-bold rounded-xl shadow-md cursor-pointer"
        >
          🛒 Add to Cart
        </button>
        <Link
